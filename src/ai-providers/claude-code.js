...
export class ClaudeCodeAIProvider extends BaseAIProvider {
	...
	detectClaudeCommand() {
		...
		// First, try 'which' command to check if claude is in PATH
		try {
	...
	getClaudeCommand(params = {}) {
		...
		if (params.modelId) {
			...
			if (rcConfig?.rawAliases && rcConfig.rawAliases[params.modelId]) {
				...
				modelToUse = params.modelId;
			}
		} else if (process.env.CLAUDE_CODE_MODEL) {
			modelToUse = process.env.CLAUDE_CODE_MODEL;
		} else if (rcConfig?.model) {
			modelToUse = rcConfig.model;
		}
	...
	async executeClaudeCode(input, options = {}) {
		...
		let command = this.getClaudeCommand(options);
		let tempFile;
		...
		try {
			...
			// Parse command into executable and args using our quote-aware parser
			const { executable, args: parsedArgs } = this.parseCommand(command);
			const claudePath = executable;
			let args = parsedArgs;
