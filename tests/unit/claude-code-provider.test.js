...
// Mock ai SDK to prevent circular dependencies
...
// Mock scripts/modules/index.js FIRST to avoid circular dependency
...
describe('Claude Code Provider Functionality', () => {
	...
	describe('Model Configuration', () => {
		...
		test('should add model flag from CLAUDE_CODE_MODEL env var', () => {
			...
			expect(command).toBe('/usr/local/bin/claude --model claude-opus-4-20250514');
		});
		...
		test('should add model flag from params.modelId', () => {
			...
			expect(command).toBe('/usr/local/bin/claude --model claude-sonnet-4-20250514');
		});
		...
		test('should handle model configuration in complex commands', () => {
			...
			expect(command).toBe(
				'/path/to/claude --mcp-config /config.json --model claude-opus-4-20250514'
			);
		});
		...
		test('should pass modelId to executeClaudeCode via generateText', async () => {
			...
			// Note: -p is added first, then model flags
		...
		test('should work with Windows-style paths and model configuration', () => {
			...
			expect(command).toBe(
				'C:\\Program Files\\Claude\\claude.exe --model claude-opus-4-20250514'
			);
		});
