...
Original PRD content...`;
	...
	describe('parseCommand - Additional Scenarios', () => {
		...
		test('should execute command with spaces in path correctly', async () => {
			// Don't include --model in CLAUDE_CODE_COMMAND
			process.env.CLAUDE_CODE_COMMAND = '"/path with spaces/claude"';
			...
			// Create a new provider instance after setting the environment variable
			const newProvider = new ClaudeCodeAIProvider();
			await newProvider.generateText({
				messages: [{ role: 'user', content: 'Test' }]
			});
			...
			expect(mockSpawn).toHaveBeenCalledWith(
				...
				expect.arrayContaining(['--model', 'claude-opus-4-20250514', '-p']),
				expect.any(Object)
			);
		});
	});
