	...
	for (const currentRole of sequence) {
		...
		try {
			...
			if (providerLower === 'claude-cli') {
				const claudeCliCommand = resolveEnvVariable('CLAUDE_CLI_COMMAND', session, effectiveProjectRoot);
				if (!claudeCliCommand) {
					log(
						'warn',
			...
			// Check if API key is set for the current provider and role (excluding 'ollama' and 'claude-cli')
